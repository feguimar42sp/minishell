...
void	run_from_root(char *command, char **cmd_flags, char **envp_arr)
{
	...
	if ((command == NULL) || (access(command, F_OK) != 0))
	{
		...
		free_split(&envp_arr);
		if (command == NULL)
			write_stderr_and_exit(" command not found", 127, &cmd_flags, &command);
		if (access(command, F_OK) != 0)
			write_stderr_and_exit(" No such file or directory", 127, &cmd_flags, &command);
	}
	...
	if ((S_ISDIR(path_data.st_mode) != 0) || (access(command, X_OK) != 0))
	{
		...
		if (access(command, X_OK) != 0)
			write_stderr_and_exit(" Permission denied", 126, &cmd_flags, &command);
	}
